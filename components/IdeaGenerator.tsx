...
// Fetch trends using API
const fetchTrends = async () => {
  try {
    const response = await fetch("http://127.0.0.1:5000/api/fetch_trends");
...
export default function IdeaGenerator() {
  ...
  const handleTrendSelect = async (value) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/api/fetch_trends`);
      ...
      alert("Failed to load trend details. Please try again later.");
    }
  };
  ...
  const handleGenerateIdeas = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("http://127.0.0.1:5000/api/generate-ideas", { // Update with your actual API endpoint
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          contentType,
          inputData,
          selectedTrend: selectedTrend ? selectedTrend.Trend : null,
        }),
      });
      ...
      const data = await response.json();
      router.push(`/generated-ideas?ideas=${encodeURIComponent(JSON.stringify(data))}`);
    } catch (error) {
            ...
            <SelectContent>
              {trends.map((trend) => (
                <SelectItem key={trend["Trend"]} value={trend["Trend"]}>
                  {trend["Trend"]} - {trend["Short Description"]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {selectedTrend && (
                      <div className="mt-4">
                        ...
                        >
                          {selectedTrend["Long Description"]}
                        </ReactMarkdown>
                      </div>
                    )}
                  </CardContent>
